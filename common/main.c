...
#if defined(CONFIG_BOOT_RETRY_TIME) && defined(CONFIG_RESET_TO_RETRY) || defined(BOARD_CHARGING)
extern int do_reset (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[]);		/* for do_reset() prototype */
#endif

extern int do_bootd (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[]);
extern u32 get_modelid(void);
...
#  endif
	...
	// this is changed from while to a do/while to check for keypress
	// when bootdelay is 0
	do {
		if (tstc()) {
...
#ifdef BOARD_CHARGING
	/* Do not increase BootCnt if rebooting from battery charging */
	if (!getenv("late_shutdown_reboot") && boot_cooldown_charger_mode() == 0) {
		bootcount++;
...
# endif	/* CONFIG_BOOT_RETRY_TIME */
	...
	if (
		...
		l = getenv("late_shutdown_reboot");
		if (l != NULL) {
			ls = simple_strtoul(l, NULL, 10);
			if (ls == 1)
				twl6030_power_off();
			else if (ls == 2)
