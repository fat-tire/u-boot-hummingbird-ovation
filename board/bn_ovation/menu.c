...
int do_menu() {
	...
	lcd_console_setpos(MENUTOP + NUM_OPTS + 12, INDENT-9);
	lcd_puts("    Press \"Volume down\" to move to the next item");
	lcd_console_setpos(MENUTOP + NUM_OPTS + 13, INDENT-9);
	lcd_puts("    Press \"Volume up\" to to move to the previous item");
	lcd_console_setpos(MENUTOP + NUM_OPTS + 14, INDENT);
	lcd_puts("    Press \"^\" to select");
	lcd_console_setpos(57, 0);
